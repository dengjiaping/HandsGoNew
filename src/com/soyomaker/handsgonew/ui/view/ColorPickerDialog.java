package com.soyomaker.handsgonew.ui.view;
...
public class ColorPickerDialog extends Dialog {
	 ...
	 *            颜色选择后的回调接口。
	 */
	public ColorPickerDialog(Context context, String title, OnColorChangedListener l) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		layout.addView(new PortraitColorPickerView(mContext, mListener),
	...
	protected abstract class ColorPickerView extends View {
		 ...
		 *            是否在渐变矩形中。
		 */
		protected void onActionMove(float x, float y, boolean inCircle, boolean inCenter,
				boolean inRect) {
			...
			if ((mHighlightCenter && inCenter) || (mlittleLightCenter && inCenter)) {// 点击中心圆,
																						// 当前移动在中心圆
	...
	private class PortraitColorPickerView extends ColorPickerView {
		...
		public PortraitColorPickerView(Context context, OnColorChangedListener l) {
			super(context, l);
			Display display = ColorPickerDialog.this.getWindow().getWindowManager()
					.getDefaultDisplay();
			...
			mCircleRadius = width / 2 * 0.7f - mCirclePaint.getStrokeWidth() * 0.5f;
		...
		protected void onDraw(Canvas canvas) {
			...
			// 画色环
			canvas.drawOval(
					new RectF(-mCircleRadius, -mCircleRadius, mCircleRadius, mCircleRadius),
					mCirclePaint);
			// 画黑白渐变块
			if (mDownInCircle) {
				mRectColors[1] = mCenterPaint.getColor();
			}
			mRectShader = new LinearGradient(mRectLeft, 0, mRectRight, 0, mRectColors, null,
					Shader.TileMode.MIRROR);
			mRectPaint.setShader(mRectShader);
			canvas.drawRect(mRectLeft, mRectTop, mRectRight, mRectBottom, mRectPaint);
			float offset = mLinePaint.getStrokeWidth() / 2;
			canvas.drawLine(mRectLeft - offset, mRectTop - offset * 2, mRectLeft - offset,
					mRectBottom + offset * 2, mLinePaint);// 左
			canvas.drawLine(mRectLeft - offset * 2, mRectTop - offset, mRectRight + offset * 2,
					mRectTop - offset, mLinePaint);// 上
			canvas.drawLine(mRectRight + offset, mRectTop - offset * 2, mRectRight + offset,
					mRectBottom + offset * 2, mLinePaint);// 右
			canvas.drawLine(mRectLeft - offset * 2, mRectBottom + offset, mRectRight + offset * 2,
					mRectBottom + offset, mLinePaint);// 下
			super.onDraw(canvas);
		}
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			boolean inCircle = inColorCircle(x, y, mCircleRadius + mCirclePaint.getStrokeWidth()
					/ 2, mCircleRadius - mCirclePaint.getStrokeWidth() / 2);
			boolean inCenter = inCenter(x, y, mCenterRadius);
			boolean inRect = inRect(x, y);
			System.out.println(x + "..." + y);
			System.out.println(mRectLeft + "..." + mRectRight + "..." + mRectTop + "..."
					+ mRectBottom);
